...
class HX711:
    ...
    def enableReadyCallback(self, paramCallback=None):
        self.paramCallback = (
            paramCallback if paramCallback is not None else self.paramCallback
        )
    ...
    def setReadingFormat(self, byteFormat="MSB", bitFormat="MSB"):
        if byteFormat != "MSB" and byteFormat != "LSB":
            raise ValueError(
                f"HX711::setReadingFormat() invalid byteFormat: '{byteFormat}'"
            )

        if bitFormat != "MSB" and bitFormat != "LSB":
            raise ValueError(
                f"HX711::setReadingFormat() invalid bitFormat: '{byteFormat}'"
            )
    ...
    def rawBytesToLong(self, rawBytes=None):
        ...
        twosComplementValue = (rawBytes[0] << 16) | (rawBytes[1] << 8) | rawBytes[2]
    ...
    def rawBytesToWeight(self, rawBytes=None, channel="A"):
        ...
        if channel == "A":
            ...
            raise ValueError(
                'HX711::rawBytesToWeight() invalid channel: "%s"' % channel
            )

        if referenceUnit == 0:
            raise ValueError(
                "HX711::rawBytesToWeight() referenceUnit is 0. It isn't possible to divide by zero!"
            )
